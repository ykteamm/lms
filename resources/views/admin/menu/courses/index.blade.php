    ...
    <div class="dashboard__main">
        <div class="dashboard__content bg-light-4">
            ...
            @endif
                ...
                <div>
                    <form action="{{route('elchi-login')}}" method="POST">
                        @csrf
                        <button type="submit">
                            OK
                        </button>
            ...
            <div class="row pb-50 mb-10">
                ...
                <div class="col-6 d-flex justify-content-end">
                        <button class="btn btn-success text-white" type="button" data-bs-toggle="modal" data-bs-target="#CourseModalCreate">
                            <i class="fas fa-plus"></i>
                            Yaratish
                        </button>

                        <div class="modal fade" id="CourseModalCreate" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    ...
                    <a href="{{route('module-index', ['course_id' => $test->id])}}" class="coursesCard -type-1 ">
                     ...
                     <div class="row py-15">
                            <div class="col-12 text-center">
                                <button class="btn btn-primary text-white" type="button" data-bs-toggle="modal" data-bs-target="#CourseModalEdit{{$test->id}}">
                                    <i class="fas fa-edit"></i>
                                    Tahrirlash
                                </button>
                                {{--                                modal edit--}}
                                <!-- Modal -->
                                <div class="modal fade" id="CourseModalEdit{{$test->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    <div class="modal-dialog modal-lg" >
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h1 class="modal-title fs-5" id="exampleModalLabel">Siz {{$test->title}} kursini tahrirlamoqchisiz!</h1>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                <form action="{{route('course.update',$test->id)}}" method="POST" class="form-group" enctype="multipart/form-data">
                                                    @csrf
                                                    @method('PUT')
                                                    <div class="row">
                                                        <div class="mb-3">
                                                            <label for="title" class="form-label fw-700">Kursni nomi</label>
                                                            <input type="text" value="{{$test->title}}" class="border form-control" id="title" name="title">
                                                            @error('title')
                                                            <div style="color: red" class="form-text">{{$message}}</div>
                                                            @enderror
                                                        </div>
                                                    @if(auth()->user()->rol_id == 'admin')
                                                        <div class="mb-3">
                                                            <label for="status" class="form-label fw-700">Kurs Status</label>
                                                            <input type="text" value="{{$test->status}}" class="border form-control" id="status" name="status">
                                                            @error('status')
                                                            <div style="color: red" class="form-text">{{$message}}</div>
                                                            @enderror
                                                        </div>
...
{{--                                </button>--}}
                                {{--                                modal delete--}}
                                <div class="modal fade" id="CourseModal{{$test->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    <div class="modal-dialog ">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h1 class="modal-title fs-5" id="exampleModalLabel">Siz {{$test->title}} kursini o'chirmoqchisiz!</h1>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                <form action="{{route('course.destroy',['course' => $test->id])}}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
